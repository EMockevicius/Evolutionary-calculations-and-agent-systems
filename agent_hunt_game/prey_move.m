...
%     if no more available moves are left - victory (for agents)
    if isempty(moves)
        fprintf("grobio ėjimų skaičius - %d\n", ejimas)
        fprintf("agentų kiekis - %d\n", AgentsN)
        fprintf("agentų ėjimų skaičius - %d\n", ejimas*AgentsN)
        fprintf("agentų atsitiktinių ėjimų skaičius - %d\n", randMoveCount)
        fprintf("agentų išėjimo už ribų skaičius - %d\n", agentDeath)
        disp('grobį pavyko pagauti'); % improve victory animation
        pause; %stabdom veikimą
    end 
end